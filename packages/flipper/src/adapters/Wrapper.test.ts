...
describe('Wrapper', () => {
  ...
  describe('delegation', () => {
    it('delegates features() to wrapped adapter', () => {
      adapter.add(feature)
      expect(wrapper.features()).toEqual([feature])
    })

    it('delegates add() to wrapped adapter', () => {
      expect(wrapper.add(feature)).toBe(true)
      expect(adapter.features()).toContainEqual(feature)
    })

    it('delegates remove() to wrapped adapter', () => {
      adapter.add(feature)
      expect(wrapper.remove(feature)).toBe(true)
      expect(adapter.features()).toHaveLength(0)
    })

    it('delegates clear() to wrapped adapter', () => {
      adapter.add(feature)
      expect(wrapper.clear(feature)).toBe(true)
    })

    it('delegates get() to wrapped adapter', () => {
      adapter.add(feature)
      const result = wrapper.get(feature)
      expect(result).toBeDefined()
    })

    it('delegates getMulti() to wrapped adapter', () => {
      adapter.add(feature)
      const result = wrapper.getMulti([feature])
      expect(result[feature.key]).toBeDefined()
    })

    it('delegates getAll() to wrapped adapter', () => {
      adapter.add(feature)
      const result = wrapper.getAll()
      expect(result[feature.key]).toBeDefined()
    })
  ...
  describe('custom wrapper', () => {
    it('allows customizing behavior via wrap() hook', () => {
      const calls: string[] = []
    ...
    it('allows modifying return values', () => {
      ...
      expect(modifying.features()).toHaveLength(0)
    })
  })
})
