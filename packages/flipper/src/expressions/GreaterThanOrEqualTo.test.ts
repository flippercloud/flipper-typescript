...
describe('GreaterThanOrEqualTo', () => {
  ...
  describe('evaluate', () => {
    ...
    test('returns true when left equals right', () => {
      const expr = new GreaterThanOrEqualTo(new Constant(5), new Constant(5))
      const context = { feature_name: 'test', properties: {} }
      expect(expr.evaluate(context)).toBe(true)
    })

    test('returns false when left is less than right', () => {
      const expr = new GreaterThanOrEqualTo(new Constant(5), new Constant(10))
      const context = { feature_name: 'test', properties: {} }
    ...
    test('works with decimal numbers', () => {
      const expr = new GreaterThanOrEqualTo(new Constant(5.5), new Constant(5.5))
      const context = { feature_name: 'test', properties: {} }
      expect(expr.evaluate(context)).toBe(true)
    })

    test('returns false when comparing null', () => {
      const expr = new GreaterThanOrEqualTo(new Constant(null), new Constant(5))
      const context = { feature_name: 'test', properties: {} }
      expect(expr.evaluate(context)).toBe(false)
    })
  ...
  describe('equals', () => {
    test('returns true for same arguments', () => {
      const expr1 = new GreaterThanOrEqualTo(new Constant(10), new Constant(5))
      const expr2 = new GreaterThanOrEqualTo(new Constant(10), new Constant(5))
      expect(expr1.equals(expr2)).toBe(true)
    })

    test('returns false for different arguments', () => {
      const expr1 = new GreaterThanOrEqualTo(new Constant(10), new Constant(5))
      const expr2 = new GreaterThanOrEqualTo(new Constant(5), new Constant(10))
